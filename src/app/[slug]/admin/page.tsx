...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (soundEnabled && newOrders > previousNewOrders.current) playNewOrderSound();
  ...
  return <main className="min-h-screen bg-[#f7f7f4] text-[#171714]">
    <header className="border-b border-black/5 bg-white"><div className="mx-auto flex flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-5 sm:py-4"><div><Link href="/" className="text-xl font-black">mambo<span className="text-[var(--brand)]">.</span> <span className="ml-2 text-sm font-medium text-[var(--muted)]">Radnik</span></Link><p className="mt-1 text-xs text-[var(--muted)]">{shop.name}</p></div><div className="flex flex-wrap items-center justify-end gap-2"><div className="flex rounded-full border border-black/10 bg-black/[.03] p-1 text-xs font-bold"><a href={`/${shop.slug}`} className="rounded-full px-3 py-1.5 text-[var(--muted)] transition hover:text-black">Kupac</a><span className="rounded-full bg-[var(--brand)] px-3 py-1.5 text-white">Radnik</span></div></div></div></header>
