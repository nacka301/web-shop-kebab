...
export default function ShopPage() {
  ...
  return <main className="min-h-screen bg-[#f7f7f4] pb-28">
    <header className="bg-[#171714] px-4 pb-7 pt-5 text-white sm:px-8 lg:px-12"><div className="mx-auto max-w-6xl"><div className="mb-8 flex flex-nowrap items-center justify-between gap-2"><Link href="/" className="shrink-0 text-2xl font-black tracking-tight" aria-label="Povratak na glavnu stranicu">mambo<span className="text-[var(--brand)]">.</span></Link><div className="flex shrink-0 items-center gap-1.5"><Link href="/" className="rounded-full border border-white/15 px-2.5 py-1.5 text-[11px] font-bold text-white/70 transition hover:bg-white/10 hover:text-white sm:px-3 sm:text-xs">Glavna</Link><div className="flex rounded-full border border-white/15 bg-white/5 p-1 text-[11px] font-bold sm:text-xs"><span className="rounded-full bg-[var(--brand)] px-2.5 py-1.5 text-white sm:px-3">Kupac</span><a href={`/${shop.slug}/admin`} className="rounded-full px-2.5 py-1.5 text-white/60 transition hover:text-white sm:px-3">Radnik</a></div></div></div><p className="mb-2 text-sm text-white/60">{shop.hours}</p><h1 className="text-3xl font-black sm:text-4xl">{shop.name}</h1><p className="mt-2 text-sm text-white/60">{shop.address}</p></div></header>
