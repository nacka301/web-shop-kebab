...
export default function ShopPage() {
  ...
  return <main className={`bg-[#f7f7f4] ${totalItems > 0 ? "pb-28" : "pb-6"}`}>
    <header className="bg-[#171714] px-4 py-5 text-white sm:px-8 lg:px-12"><div className="mx-auto max-w-6xl"><div className="flex flex-nowrap items-center justify-between gap-2"><Link href="/" className="shrink-0 text-2xl font-black tracking-tight" aria-label="Povratak na glavnu stranicu">mambo<span className="text-[var(--brand)]">.</span></Link><div className="flex shrink-0 items-center"><div className="flex rounded-full border border-white/15 bg-white/5 p-1 text-[11px] font-bold sm:text-xs"><span className="rounded-full bg-[var(--brand)] px-2.5 py-1.5 text-white sm:px-3">Kupac</span><Link href={`/${shop.slug}/admin`} className="rounded-full px-2.5 py-1.5 text-white/60 transition hover:text-white sm:px-3">Radnik</Link></div></div></div></div></header>
    <section className="sticky top-0 z-10 relative border-b border-black/5 bg-[#f7f7f4]/95 backdrop-blur"><div className="mx-auto flex max-w-6xl gap-2 overflow-x-auto px-5 py-4 sm:px-8 lg:px-12">{categories.map((item) => <button key={item} onClick={() => setCategory(item)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold ${category === item ? "bg-black text-white" : "bg-white text-[var(--muted)]"}`}>{item}</button>)}</div><div className="pointer-events-none absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-[#f7f7f4] to-transparent sm:hidden" /></section>
