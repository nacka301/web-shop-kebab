...
export default function ShopPage() {
  ...
  const [orderNumber] = useState(() => `#${1049 + Math.floor(Math.random() * 50)}`);
  ...
  if (submitted) {
    return <main className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-6 text-center"><div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-green-100 text-4xl">✓</div><p className="mb-2 text-sm font-bold uppercase tracking-[0.2em] text-[var(--brand)]">Narudžba zaprimljena</p><h1 className="mb-3 text-4xl font-black">Hvala, {customer.name.split(" ")[0]}!</h1><p className="mb-3 text-[var(--muted)]">Tvoja narudžba je potvrđena.</p><p className="mb-2 text-sm font-bold text-[var(--muted)]">Broj narudžbe</p><div className="mb-8 rounded-2xl bg-white px-10 py-5 text-4xl font-black shadow-sm">{orderNumber}</div><p className="mb-8 text-sm text-[var(--muted)]">Preuzimanje: <strong className="text-black">{customer.pickup}</strong><br />Plaćanje prilikom preuzimanja.</p><a className="w-full rounded-xl bg-[var(--brand)] px-5 py-4 font-bold text-white" href={`/${shop.slug}`}>Natrag na meni</a></main>;
  }

  return <main className="min-h-screen bg-[#f7f7f4] pb-28">
    <header className="bg-[#171714] px-5 pb-7 pt-6 text-white sm:px-8 lg:px-12"><div className="mx-auto max-w-6xl"><div className="mb-9 flex items-center justify-between"><div className="text-2xl font-black tracking-tight">mambo<span className="text-[var(--brand)]">.</span></div><a href={`/${shop.slug}/admin`} className="rounded-full border border-white/20 px-3 py-1.5 text-xs font-semibold text-white/70">Radnici</a></div><p className="mb-2 text-sm text-white/60">{shop.hours}</p><h1 className="text-3xl font-black sm:text-4xl">{shop.name}</h1><p className="mt-2 text-sm text-white/60">{shop.address}</p></div></header>
    ...
    <section className="mx-auto grid max-w-6xl gap-3 px-5 py-5 sm:grid-cols-2 sm:px-8 lg:grid-cols-3 lg:gap-5 lg:px-12">{visible.map((item) => <article key={item.id} className="flex gap-3 rounded-2xl bg-white p-3 shadow-sm sm:p-4"><div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-xl bg-[#fff0e8] text-4xl sm:h-24 sm:w-24">{item.emoji}</div><div className="min-w-0 flex-1"><h2 className="font-bold">{item.name}</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">{item.description}</p><div className="mt-2 flex items-center justify-between"><strong>{money(item.price)}</strong><button onClick={() => openCustomization(item)} className="rounded-lg bg-[#fff0e8] px-3 py-1.5 text-sm font-bold text-[var(--brand-dark)]">+ Dodaj</button></div></div></article>)}</section>
