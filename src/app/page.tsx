            ...
            <div className="relative mx-auto max-w-xl overflow-hidden rounded-3xl border border-black/5 bg-white shadow-xl">
              ...
              <div className={`bg-[#f7f7f4] p-4 sm:p-5 ${previewCount > 0 ? "pb-20" : ""}`}>
                ...
                <div className="space-y-3">
                  {menu.slice(0, 3).map((item) => <div key={item.id} className="flex items-center gap-3 rounded-2xl bg-white p-3 shadow-sm"><span className="flex h-14 w-14 items-center justify-center rounded-xl bg-[#fff0e8] text-3xl">{item.emoji}</span><div className="min-w-0 flex-1"><p className="font-bold">{item.name}</p><p className="mt-1 truncate text-xs text-[var(--muted)]">{item.description}</p><p className="mt-1 text-sm font-black">{item.price.toFixed(2).replace(".", ",")} €</p></div><button onClick={() => togglePreviewItem(item.id)} className={`rounded-lg px-3 py-2 text-xs font-bold ${previewAdded[item.id] ? "bg-[var(--brand)] text-white" : "bg-[#fff0e8] text-[var(--brand-dark)]"}`}>{previewAdded[item.id] ? "✓ Dodano" : "+ Dodaj"}</button></div>)}
                </div>
